...
const hourFmt = computed(
  () => new Intl.DateTimeFormat(props.locale ?? 'en', { hour: 'numeric' }),
);
...
const peakLabel = (weekday: number, bucketIdx: number) =>
  `${DAYS[weekday]} ${BUCKETS[bucketIdx].label.toLowerCase()}`;
</script>
  ...
  <WidgetFrame>
    ...
    <div class="umami-heatmap__grid-wrap">
      ...
      <div v-for="(day, di) in DAYS" :key="day" class="umami-heatmap__row">
        ...
        <Tooltip
          v-for="(_, bi) in BUCKETS"
          :key="bi"
          :text="bucketTooltip(di, bi)"
        >
          <div
            class="umami-heatmap__cell"
            :class="{ 'umami-heatmap__cell--skeleton': !data }"
...
.umami-heatmap__row:nth-child(2) .umami-heatmap__cell--skeleton { animation-delay: 0ms; }
.umami-heatmap__row:nth-child(3) .umami-heatmap__cell--skeleton { animation-delay: 80ms; }
.umami-heatmap__row:nth-child(4) .umami-heatmap__cell--skeleton { animation-delay: 160ms; }
.umami-heatmap__row:nth-child(5) .umami-heatmap__cell--skeleton { animation-delay: 240ms; }
.umami-heatmap__row:nth-child(6) .umami-heatmap__cell--skeleton { animation-delay: 320ms; }
.umami-heatmap__row:nth-child(7) .umami-heatmap__cell--skeleton { animation-delay: 400ms; }
.umami-heatmap__row:nth-child(8) .umami-heatmap__cell--skeleton { animation-delay: 480ms; }
